...
// ProductName reports the name of the Nxpkg product that is currently running ("Nxpkg
// Server", "Nxpkg Data Center", etc.).
func ProductName() string {
	deployType := DeployType()
	switch {
	case IsDataCenter(deployType):
		return "Nxpkg Data Center"
...
// Nxpkg API without requiring users to take manual steps.
//
...
func SupportsManagingLanguageServers() (reason string, ok bool) {
	...
	if IsDataCenter(deployType) {
		...
		return "Managing language servers automatically is not supported in Nxpkg Data Center. See https://github.com/nxpkg/deploy-nxpkg/blob/master/docs/install.md#add-language-servers-for-code-intelligence for help.", false
	}
