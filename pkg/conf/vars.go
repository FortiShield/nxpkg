...
var configVarsEnabled, _ = strconv.ParseBool(env.Get("NXPKG_EXPAND_CONFIG_VARS", "false", "expand ${var} and $var in site configuration JSON based on the environment (env vars)"))

// expandEnv replaces $var or ${var} based on environment variable values. See the docstring of the
// expand func for details.
//
// If NXPKG_EXPAND_CONFIG_VARS is disabled, it returns the input unchanged and a nil error.
...
//
// This func implements NXPKG_EXPAND_CONFIG_VARS. Callers should use expandEnv instead of this
// low-level func.
