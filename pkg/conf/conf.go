...
//
// If using NXPKG_EXPAND_CONFIG_VARS, this value is NOT expanded. Callers must expand env vars
...
func init() {
	configFilePath = os.Getenv("NXPKG_CONFIG_FILE")

	// Read configuration initially.
	if err := initConfig(false); err != nil {
		log.Fatalf("failed to read configuration from environment: %s. Fix your Nxpkg configuration (%s) to resolve this error. Visit https://about.nxpkg.com/docs to learn more.", err, configFilePath)
	}
		...
		for {
			...
			if IsDirty() {
				...
				if err := initConfig(true); err != nil {
					log.Printf("failed to read configuration from environment: %s. Fix your Nxpkg configuration (%s) to resolve this error. Visit https://about.nxpkg.com/docs to learn more.", err, configFilePath)
				}
