...
func httpGet(ctx context.Context, op, urlStr string, result interface{}) (err error) {
	...
	req.Header.Set("User-Agent", "Nxpkg registry client v"+APIVersion)
	...
	if v := strings.TrimSpace(resp.Header.Get(MediaTypeHeaderName)); v != MediaType {
		return &url.Error{Op: op, URL: urlStr, Err: fmt.Errorf("not a valid Nxpkg registry (invalid media type %q, expected %q)", v, MediaType)}
	}
