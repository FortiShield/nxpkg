...
import (
	...
	"github.com/nxpkg/nxpkg/xlang/lspext"
)

var (
	url         = flag.String("url", "ws://localhost:3080/.api/lsp", "WebSocket LSP gateway URL (ws:// or wss://)") // CI:LOCALHOST_OK
	accessToken = flag.String("token", "", "Nxpkg API access token")
	mode        = flag.String("mode", "go", "")
	rootURI     = flag.String("root-uri", "git://github.com/nxpkg/go-diff?3f415a150aec0685cb81b73cc201e762e075006d", "LSP initialize rootURI (git://repo?sha)")
...
func main() {
	...
	if *accessToken == "" {
		log.Fatalf("Must set -token flag to Nxpkg API access token valid for %s", *url)
	}
