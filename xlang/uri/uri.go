...
func Parse(uriStr string) (*URI, error) {
	...
	if !u.IsAbs() {
		return nil, &url.Error{Op: "uri.Parse", URL: uriStr, Err: errors.New("nxpkg URI must be absolute")}
	}
	return &URI{*u}, nil
}
...
func (u *URI) Repo() api.RepoURI { return api.RepoURI(u.Host + strings.TrimPrefix(u.Path, ".git")) }
