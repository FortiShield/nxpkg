...
import (
	...
	"github.com/nxpkg/nxpkg/cmd/query-runner/queryrunnerapi"
...
func serveTestNotification(w http.ResponseWriter, r *http.Request) {
	...
	for _, recipient := range recipients {
		...
		if err := slackNotify(context.Background(), recipient,
			fmt.Sprintf(`It worked! This is a test notification for the Nxpkg saved search <%s|"%s">.`, searchURL(query.Config.Query, utmSourceSlack), query.Config.Description)); err != nil {
			writeError(w, fmt.Errorf("error sending email notifications to %s: %s", recipient.spec, err))
			return
		}
	}
