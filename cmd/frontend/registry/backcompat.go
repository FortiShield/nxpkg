...
import (
	...
	"github.com/nxpkg/nxpkg/cmd/frontend/internal/pkg/langservers"
...
func init() {
	...
	conf.Watch(func() {
		...
		for _, ls := range enabledLangServers {
			...
			readme += `This extension was automatically created from the Nxpkg site configuration's ` + "`" + `langservers.` + ls.Language + "`" + ` setting. Site admins may delete this extension by removing that setting from site configuration.` + "\n\n"
			if info != nil {
...
* [Issue tracker](` + info.IssuesURL + `)`
			...
			x := schema.NxpkgExtensionManifest{
				Title:       title,
				Description: description,
				Readme:      readme,
				// The same extension is used for each language server (for now). It is built from
				// https://github.com/nxpkg/nxpkg-langserver-http.
				Url:              "https://storage.googleapis.com/nxpkg-cx-dev/nxpkg-langserver-http.3.js",
