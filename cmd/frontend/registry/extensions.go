...
import (
	...
	features "github.com/nxpkg/nxpkg/cmd/frontend/features"
	...
	"github.com/nxpkg/nxpkg/pkg/httputil"
...
// "nxpkg.example.com/bob/myextension". It returns the components, or a non-nil error if
// parsing failed.
...
// optional registry prefix), such as "alice/myextension" or
// "nxpkg.example.com/bob/myextension". It validates that the registry prefix is correct given
...
func ParseExtensionID(extensionID string) (prefix, extensionIDWithoutPrefix string, isLocal bool, err error) {
	...
	if prefix != "" {
		...
		if configuredPrefix == nil {
			// Don't look up fully qualified extensions from Nxpkg.com; it only cares about
			// its own extensions.
			return "", "", false, fmt.Errorf("remote extension lookup is not supported for host %q", prefix)
		}

		// Local extension on non-Nxpkg.com instance.
		...
		if configuredPrefix == nil {
			// Local extension on Nxpkg.com instance.
			isLocal = true
		}
	}
...
// to the remote registry specified in site configuration (usually nxpkg.com). The host must
// be specified to refer to a local extension on the current Nxpkg site (e.g.,
// nxpkg.example.com/publisher/name).
//
...
// Nxpkg's communication with the remote extension registry (usually Nxpkg.com) does not
