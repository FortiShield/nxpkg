...
import (
	...
	"github.com/nxpkg/nxpkg/cmd/frontend/db/migrations"
...
func ConnectToDB(dataSource string) error {
	...
	if err := DoMigrateAndClose(NewMigrate(Global)); err != nil {
		return errors.Wrap(err, "Failed to migrate the DB. Please contact support@nxpkg.com for further assistance")
	}
...
func DoMigrateAndClose(m *migrate.Migrate) (err error) {
	...
	if os.IsNotExist(err) {
		...
		log15.Warn("WARNING: Detected an old version of Nxpkg. The database has migrated to a newer version. If you have applied a rollback, this is expected and you can ignore this warning. If not, please contact support@nxpkg.com for further assistance.", "db_version", version)
		return nil
	}
	return err
}
