// The testproxy command runs a simple HTTP proxy that wraps a Nxpkg server running with site
...
var (
	...
	urlStr     = flag.String("url", "http://localhost:3080", "proxy origin URL (Nxpkg HTTP/HTTPS URL)") // CI:LOCALHOST_OK
	username   = flag.String("username", os.Getenv("USER"), "username to report to Nxpkg")
