...
func CreateOrUpdateUser(ctx context.Context, newOrUpdatedUser db.NewUser, externalAccount db.ExternalAccountSpec, data db.ExternalAccountData) (userID int32, safeErrMsg string, err error) {
	...
	if actor := actor.FromContext(ctx); actor.IsAuthenticated() {
		...
		if err := db.ExternalAccounts.AssociateUserAndSave(ctx, userID, externalAccount, data); err != nil {
			safeErrMsg = "Unexpected error associating the external account with your Nxpkg user. The most likely cause for this problem is that another Nxpkg user is already linked with this external account. A site admin or the other user can unlink the account to fix this problem."
			return 0, safeErrMsg, err
		}
	} else {
		userID, err = db.ExternalAccounts.LookupUserAndSave(ctx, externalAccount, data)
		if err != nil && !errcode.IsNotFound(err) {
			return 0, "Unexpected error looking up the Nxpkg user account associated with the external account. Ask a site admin for help.", err
		}
		if errcode.IsNotFound(err) {
			...
			case db.IsUsernameExists(err):
				...
				safeErrMsg = fmt.Sprintf("The Nxpkg username %q already exists and is not linked to this external account. If possible, sign using the external account you used previously. If that's not possible, a site admin can unlink or delete the Nxpkg account with that username to fix this problem.", newOrUpdatedUser.Username)
			case db.IsEmailExists(err):
				...
				safeErrMsg = fmt.Sprintf("The email address %q already exists and is associated with a different Nxpkg user. A site admin can remove the email address from that Nxpkg user to fix this problem.", newOrUpdatedUser.Email)
			...
			if associateUser {
				if err := db.ExternalAccounts.AssociateUserAndSave(ctx, userID, externalAccount, data); err != nil {
					safeErrMsg = "Unexpected error associating the external account with the existing Nxpkg user with the same username or email address."
					return 0, safeErrMsg, err
				}
			}
	...
	// which the user is somewhat likely to want to control separately on Nxpkg.)
	user, err := db.Users.GetByID(ctx, userID)
	if err != nil {
		return 0, "Unexpected error getting the Nxpkg user account. Ask a site admin for help.", err
	}
	...
	if userUpdate != (db.UserUpdate{}) {
		if err := db.Users.Update(ctx, user.ID, userUpdate); err != nil {
			return 0, "Unexpected error updating the Nxpkg user account with new user profile information from the external account. Ask a site admin for help.", err
		}
	}
	return user.ID, "", nil
}
