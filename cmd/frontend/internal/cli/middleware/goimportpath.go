...
// NxpkgComGoGetHandler is middleware for serving go-import meta tags for requests with ?go-get=1 query
// on nxpkg.com.
//
// It implements the following mapping:
//
// 1. If the username (first path element) is "nxpkg", consider it to be a vanity
//    import path pointing to github.com/nxpkg/<repo> as the clone URL.
...
func NxpkgComGoGetHandler(next http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		// Handle "go get nxpkg.com/{nxpkg,sqs}/*" for all non-hosted repositories.
		// It's a vanity import path that maps to "github.com/{nxpkg,sqs}/*" clone URLs.
		pathElements := strings.Split(req.URL.Path[1:], "/")
		if len(pathElements) >= 2 && (pathElements[0] == "nxpkg" || pathElements[0] == "sqs") {
