...
func checkSysReqs(ctx context.Context, w io.Writer) error {
	...
	for _, st := range sysreq.Check(ctx, skippedSysReqs()) {
		if st.Failed() {
			...
			fmt.Fprintln(w, "\t"+wrap(fmt.Sprintf("Skip this check by setting the env var %s=%q (separate multiple entries with spaces). Note: Nxpkg may not function properly without %s.", skipSysReqsEnvVar, st.Name, st.Name)))
		}
	}
