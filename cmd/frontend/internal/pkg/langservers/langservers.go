...
type StaticInfoT struct {
	...
	//
	// TODO: Remove the need for this: https://github.com/nxpkg/nxpkg/issues/10693
	kill bool
...
var StaticInfo = map[string]*StaticInfoT{
	"go": {
		DisplayName:  "Go",
		HomepageURL:  "https://github.com/nxpkg/go-langserver",
		IssuesURL:    "https://github.com/nxpkg/go-langserver/issues",
		DocsURL:      "https://github.com/nxpkg/go-langserver/blob/master/README.md",
		Experimental: false,
		SiteConfig:   schema.Langservers{Language: "go", Address: "tcp://go:4389"},
	},
	"typescript": {
		DisplayName:  "TypeScript",
		HomepageURL:  "https://github.com/nxpkg/javascript-typescript-langserver",
		IssuesURL:    "https://github.com/nxpkg/javascript-typescript-langserver/issues",
		DocsURL:      "https://github.com/nxpkg/javascript-typescript-langserver/blob/master/README.md",
	...
	"python": {
		DisplayName:  "Python",
		HomepageURL:  "https://github.com/nxpkg/python-langserver",
		IssuesURL:    "https://github.com/nxpkg/python-langserver/issues",
		DocsURL:      "https://github.com/nxpkg/python-langserver/blob/master/README.md",
		SiteConfig:   schema.Langservers{Language: "python", Address: "tcp://python:2087"},
		Experimental: false,
	},
	"java": {
		DisplayName:  "Java",
		HomepageURL:  "https://github.com/nxpkg/java-langserver-docs",
		IssuesURL:    "https://github.com/nxpkg/java-langserver-docs/issues",
		DocsURL:      "https://github.com/nxpkg/java-langserver-docs/blob/master/README.md",
...
func start(language string) error {
	...
	// 	$ docker ps
	// 	[...] IMAGE                               STATUS                         NAMES
	// 	[...] nxpkg/codeintel-typescript    Exited (137) 32 minutes ago    typescript
	//
	// 	$ docker run --name=typescript --restart=always nxpkg/codeintel-typescript
	...
	// container names that are unlikely to conflict like "nxpkg-typescript" or explore
	// DIND (https://github.com/nxpkg/nxpkg/issues/11616). Note however that you
	// cannot `docker rm` a running container, so the issue would be in Nxpkg and not
...
func startProdArgs(language string) (args []string) {
	if language == "go" {
		args = append(args, []string{"-e", "SRC_GIT_SERVERS=nxpkg:3178"}...)
	}
	return args
}
...
// multiLanguageProviders maps a single nxpkg/codeintel-$KEY container to the
// multiple languages it provides. For example, nxpkg/codeintel-typescript
...
var multiLanguageProviders = map[string][]string{
	// nxpkg/codeintel-typescript provides "typescript" and "javascript" languages.
	"typescript": []string{
		"typescript",
		"javascript",
	},
...
// "nxpkg/codeintel-javascript" does not exist but rather is the same
// "nxpkg/codeintel-typescript" Docker image & container.
...
func imageName(language string) string {
	switch language {
	case "cs":
		return "nxpkg/codeintel-csharp"
	case "dockerfile":
		return "nxpkg/codeintel-docker"
	default:
		return "nxpkg/codeintel-" + language
	}
}
...
// Nxpkg container (for security reasons).
...
func init() {
	...
	// 	  Nxpkg access to manage my Docker containers!") and removing
	...
	if !haveSocket {
		const msg = "Language server management capabilities disabled because /var/run/docker.sock was not found. See https://about.nxpkg.com/docs/code-intelligence/install for help."
...
// nxpkg/server container and language server containers.
//
...
func setupNetworking() {
	...
	//    would trigger this situation much more frequently (see https://github.com/nxpkg/nxpkg/pull/10600)
	//
	// 2. When the network does become bugged, it is still connected but
	//    experiences EXTREME packet loss (about 79%), i.e. `docker exec nxpkg ping go`
...
// setContainerID changes the name of our container to "nxpkg" so that it is
...
func setContainerID() {
	...
	_, err = dockerCmd("container", "rename", containerID, "nxpkg")
	if err != nil {
...
// createLSPBridge creates a bridge network for the language servers to communicate with nxpkg/server.
func createLSPBridge() {
	...
	_, err = dockerCmd("network", "connect", "lsp", "nxpkg")
	if err != nil {
