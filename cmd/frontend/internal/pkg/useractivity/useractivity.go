...
// individual and aggregate Nxpkg users' activity levels.
//
// Note that this package should not be used on nxpkg.com, only on self-hosted
// deployments.
...
import (
	...
	"github.com/nxpkg/nxpkg/pkg/redispool"
...
func uniques(dayStart time.Time, period *UsageDuration) (*ActiveUsers, error) {
	...
	for d.After(dayStart) || d.Equal(dayStart) {
		...
		for _, id := range values {
			...
			if len(bid) != 36 { // id is a numerical Nxpkg user id, not an anonymous user's UUID
