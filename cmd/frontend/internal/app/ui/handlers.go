...
import (
	...
	"github.com/nxpkg/nxpkg/cmd/frontend/internal/app/jscontext"
	...
	"github.com/nxpkg/nxpkg/pkg/vcs"
)
...
type Metadata struct {
	...
	// etc. e.g. "View this link in Nxpkg Editor."
	Description string
}
...
func newCommon(w http.ResponseWriter, r *http.Request, title string, serveError func(w http.ResponseWriter, r *http.Request, err error, statusCode int)) (*Common, error) {
	...
	common := &Common{
		...
		InjectNxpkgTracker: injectTelligentTracker,
	}
	...
	if _, ok := mux.Vars(r)["Repo"]; ok {
		...
		if common.Repo.URI == "github.com/nxpkgtest/Always500Test" {
...
func serveHome(w http.ResponseWriter, r *http.Request) error {
	common, err := newCommon(w, r, "Nxpkg", serveError)
	if err != nil {
		return err
	}
	if common == nil {
	...
	if (r.Host == "nxpkg.com" || r.Host == "www.nxpkg.com") && !actor.FromContext(r.Context()).IsAuthenticated() {
		// The user is not signed in and tried to access our main site at nxpkg.com.
		// Redirect to about.nxpkg.com so they see general info.
	...
	// nxpkg.com (not about) homepage. There is none, redirect them to /search.
	r.URL.Path = "/search"
...
// https://github.com/nxpkg/search-badger
