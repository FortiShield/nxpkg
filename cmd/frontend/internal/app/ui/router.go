...
import (
	...
	uirouter "github.com/nxpkg/nxpkg/cmd/frontend/internal/app/ui/router"
	"github.com/nxpkg/nxpkg/pkg/randstring"
	"github.com/nxpkg/nxpkg/pkg/routevar"
...
const (
	...
	aboutRedirectHost   = "about.nxpkg.com"
	routeSurvey         = "survey"
...
// nxpkg.com/$KEY should redirect to about.nxpkg.com/$VALUE.
...
func initRouter() {
	...
	router.Get(routeThreads).Handler(handler(serveBasicPageString("Threads - Nxpkg")))
	router.Get(uirouter.RouteSignIn).Handler(handler(serveBasicPageString("Sign in - Nxpkg")))
	router.Get(uirouter.RouteSignUp).Handler(handler(serveBasicPageString("Sign up - Nxpkg")))
	router.Get(routeOrganizations).Handler(handler(serveBasicPageString("Organization - Nxpkg")))
	router.Get(routeSettings).Handler(handler(serveBasicPageString("Settings - Nxpkg")))
	router.Get(routeSiteAdmin).Handler(handler(serveBasicPageString("Admin - Nxpkg")))
	router.Get(uirouter.RoutePasswordReset).Handler(handler(serveBasicPageString("Reset password - Nxpkg")))
	router.Get(routeExplore).Handler(handler(serveBasicPageString("Explore - Nxpkg")))
	router.Get(routeDiscussions).Handler(handler(serveBasicPageString("Discussions - Nxpkg")))
	router.Get(routeAPIConsole).Handler(handler(serveBasicPageString("API explorer - Nxpkg")))
	router.Get(routeRepoSettings).Handler(handler(serveBasicPageString("Repository settings - Nxpkg")))
	router.Get(routeRepoCommit).Handler(handler(serveBasicPageString("Commit - Nxpkg")))
	router.Get(routeRepoBranches).Handler(handler(serveBasicPageString("Branches - Nxpkg")))
	router.Get(routeRepoCommits).Handler(handler(serveBasicPageString("Commits - Nxpkg")))
	router.Get(routeRepoTags).Handler(handler(serveBasicPageString("Tags - Nxpkg")))
	router.Get(routeRepoCompare).Handler(handler(serveBasicPageString("Compare - Nxpkg")))
	router.Get(routeRepoStats).Handler(handler(serveBasicPageString("Stats - Nxpkg")))
	router.Get(routeRepoGraph).Handler(handler(serveBasicPageString("Repository graph - Nxpkg")))
	router.Get(routeSearchScope).Handler(handler(serveBasicPageString("Search scope - Nxpkg")))
	router.Get(routeSurvey).Handler(handler(serveBasicPageString("Survey - Nxpkg")))
	router.Get(routeSurveyScore).Handler(handler(serveBasicPageString("Survey - Nxpkg")))
	router.Get(routeRegistry).Handler(handler(serveBasicPageString("Registry - Nxpkg")))
	router.Get(routeExtensions).Handler(handler(serveBasicPageString("Extensions - Nxpkg")))

	router.Get(routeUserSettings).Handler(handler(serveBasicPageString("User settings - Nxpkg")))
	router.Get(routeUser).Handler(handler(serveBasicPage(func(c *Common, r *http.Request) string {
		return mux.Vars(r)["username"] + " - Nxpkg"
	})))

	// Legacy redirects
	if envvar.NxpkgDotComMode() {
		router.Get(routeLegacyLogin).Handler(staticRedirectHandler("/sign-in", http.StatusMovedPermanently))
		router.Get(routeLegacyCareers).Handler(staticRedirectHandler("https://about.nxpkg.com/jobs", http.StatusMovedPermanently))
	...
	router.Get(routeSearch).Handler(handler(serveBasicPage(func(c *Common, r *http.Request) string {
		...
		if shortQuery == "" {
			return "Nxpkg" // no query, on search homepage
		}
		// e.g. "myquery - Nxpkg"
		return fmt.Sprintf("%s - Nxpkg", shortQuery)
	})))
	...
	serveRepoHandler := handler(serveRepoOrBlob(routeRepo, func(c *Common, r *http.Request) string {
		// e.g. "gorilla/mux - Nxpkg"
		return fmt.Sprintf("%s - Nxpkg", repoShortName(c.Repo.URI))
	}))
	...
	router.Get(routeTree).Handler(handler(serveBasicPage(func(c *Common, r *http.Request) string {
		// e.g. "src - gorilla/mux - Nxpkg"
		dirName := path.Base(mux.Vars(r)["Path"])
		return fmt.Sprintf("%s - %s - Nxpkg", dirName, repoShortName(c.Repo.URI))
	})))

	// blob
	router.Get(routeBlob).Handler(handler(serveRepoOrBlob(routeBlob, func(c *Common, r *http.Request) string {
		// e.g. "mux.go - gorilla/mux - Nxpkg"
		fileName := path.Base(mux.Vars(r)["Path"])
		return fmt.Sprintf("%s - %s - Nxpkg", fileName, repoShortName(c.Repo.URI))
	})))
...
// request. For example:
//
// 	staticRedirectHandler("http://google.com") serving "https://nxpkg.com/foobar?q=foo" -> "http://google.com/foobar?q=foo"
// 	staticRedirectHandler("/foo") serving "https://nxpkg.com/bar?q=foo" -> "https://nxpkg.com/foo?q=foo"
//
...
//
// See https://github.com/nxpkg/nxpkg/issues/9453
...
func serveErrorNoDebug(w http.ResponseWriter, r *http.Request, err error, statusCode int, nodebug, forceServeError bool) {
	...
	title := fmt.Sprintf("%v %s - Nxpkg", statusCode, http.StatusText(statusCode))
