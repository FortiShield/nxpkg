...
func TestLatestServerVersionPushed(t *testing.T) {
	...
	url := fmt.Sprintf("https://index.docker.io/v1/repositories/nxpkg/server/tags/%s", latestReleaseServerBuild.Version)
	resp, err := http.Get(url)
	if err != nil {
		t.Skip("Failed to contact dockerhub", err)
	}
	if resp.StatusCode == 404 {
		t.Fatalf("nxpkg/server:%s does not exist on dockerhub. %s", latestReleaseServerBuild.Version, url)
	}
...
func TestLatestDataCenterVersionPushed(t *testing.T) {
	...
	url := fmt.Sprintf("https://github.com/nxpkg/deploy-nxpkg/releases/tag/v%v", latestReleaseDataCenterBuild.Version)
