...
func quickGitserverRepo(repo api.RepoURI) *gitserver.Repo {
	...
	// (especially on Nxpkg.com), which reduces rate limit pressure significantly.
	//
...
func maybeLogRepoUpdaterError(repo *types.Repo, err error) {
	...
	case repoupdater.ErrNotFound:
		msg = "Repository host reported a repository as not found. If this repository was deleted on its origin, the site admin must explicitly delete it on Nxpkg."
