...
func (userEmails) Add(ctx context.Context, userID int32, email string) error {
	...
	if isSiteAdmin := CheckCurrentUserIsSiteAdmin(ctx) == nil; !isSiteAdmin {
		...
		if envvar.NxpkgDotComMode() {
			// Abuse prevention check 3: Set a quota on Nxpkg.com users to prevent abuse.
			//
...
var (
	verifyEmailTemplates = txemail.MustValidate(txemail.Templates{
		Subject: `Verify your email on Nxpkg`,
		Text: `
Verify your email address {{printf "%q" .Email}} on Nxpkg by following this link:

  {{.URL}}
`,
		HTML: `
<p>Verify your email address {{printf "%q" .Email}} on Nxpkg by following this link:</p>
