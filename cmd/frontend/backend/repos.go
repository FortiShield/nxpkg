...
import (
	...
	"github.com/nxpkg/nxpkg/pkg/inventory"
	"github.com/nxpkg/nxpkg/pkg/rcache"
	"github.com/nxpkg/nxpkg/pkg/repoupdater"
	"github.com/nxpkg/nxpkg/pkg/repoupdater/protocol"
	"github.com/nxpkg/nxpkg/pkg/vcs/git"
)

// ErrRepoSeeOther indicates that the repo does not exist on this server but might exist on an external Nxpkg
// server.
...
func (s *repos) GetByURI(ctx context.Context, uri api.RepoURI) (_ *types.Repo, err error) {
	...
	if err != nil && envvar.NxpkgDotComMode() {
		// Automatically add repositories on Nxpkg.com.
...
func (s *repos) Add(ctx context.Context, uri api.RepoURI) (err error) {
	...
	if result.Repo != nil {
		// Allow anonymous users on Nxpkg.com to enable repositories just by visiting them, but
		// everywhere else, require server admins to explicitly enable repositories.
		enableAutoAddedRepos := envvar.NxpkgDotComMode()
