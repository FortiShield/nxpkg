...
type Mutation {
    ...
    # host, or else Nxpkg won't know how to clone it.
    #
    ...
    # for storing (it is not accessible by Nxpkg after creation).
    #
    ...
    # Deletes the association between an external account and its Nxpkg user. It does NOT delete the external
    ...
    #
    # Tags are used internally by Nxpkg as feature flags for experimental features.
    #
    # Only site admins may perform this mutation.
    setTag(
    ...
    # Adds a Phabricator repository to Nxpkg.
    addPhabricatorRepo(
    ...
    resolvePhabricatorDiff(
        ...
        # Required if Nxpkg doesn't have a Conduit API token.
        patch: String
    ...
    # Mutations that are only used on Nxpkg.com.
    #
...
input SurveySubmissionInput {
    ...
    # will not be used.
    email: String
    # User's likelihood of recommending Nxpkg to a friend, from 0-10.
    score: Int!
    # The answer to "What is the most important reason for the score you gave".
    reason: String
    # The answer to "What can Nxpkg do to provide a better product"
    better: String
}
...
type ClientConfigurationDetails {
    ...
    # Returns details about the parent Nxpkg instance.
    parentNxpkg: ParentNxpkgDetails!
}

# Parent Nxpkg instance
type ParentNxpkgDetails {
    # Nxpkg instance URL.
    url: String!
}
...
type Repository implements Node {
    ...
    enabled: Boolean!
    # The date when this repository was created on Nxpkg.
    createdAt: String!
    # The date when this repository's metadata was last updated on Nxpkg.
    updatedAt: String
    ...
    # some code host). In this case, the remote source repository is external to Nxpkg and the mirror is
    # maintained by the Nxpkg site (not the other way around).
    ...
    # Link to another Nxpkg instance location where this repository is located.
    redirectURL: String
...
# is external to Nxpkg and the mirror is maintained by the Nxpkg site (not the other way
# around).
...
# A file.
#
# In a future version of Nxpkg, a repository's files may be distinct from a repository's blobs
...
type User implements Node & ConfigurationSubject {
    ...
    nxpkgID: Int! @deprecated(reason: "use id instead")
    ...
    avatarURL: String
    # The URL to the user's profile on Nxpkg.
    url: String!
    # The URL to the user's settings.
    settingsURL: String!
    # The date when the user account was created on Nxpkg.
    createdAt: String!
    # The date when the user account was last updated on Nxpkg.
    updatedAt: String
    ...
    # Only the user and site admins can access this field.
    tags: [String!]!
    # The user's usage activity on Nxpkg.
    #
    # Only the user and site admins can access this field.
...
type ExternalAccount implements Node {
    ...
    accountID: String!
    # The creation date of this external account on Nxpkg.
    createdAt: String!
    # The last-updated date of this external account on Nxpkg.
    updatedAt: String!
...
type Session {
    # Whether the user can sign out of this session on Nxpkg.
    canSignOut: Boolean!
}
...
type Org implements Node & ConfigurationSubject {
    ...
    id: ID!
    # The organization's name. This is unique among all organizations on this Nxpkg site.
    name: String!
...
# A site is an installation of Nxpkg that consists of one or more
...
type Site implements ConfigurationSubject {
    ...
    # The name of the Nxpkg product that is used on this site ("Nxpkg Server" or "Nxpkg Data
    # Center" when running in production).
    productName: String!
    # The build version of the Nxpkg software that is running on this site (of the form
    # NNNNN_YYYY-MM-DD_XXXXX, like 12345_2018-01-01_abcdef).
    buildVersion: String!
    # The product version of the Nxpkg software that is running on this site.
    productVersion: String!
    # Information about software updates for the version of Nxpkg that this site is running.
...
type SiteConfiguration {
    ...
    # referring to either the on-disk file path or the NXPKG_CONFIG
    # env var.
    source: String!
}

# Information about software updates for Nxpkg.
type UpdateCheck {
...
#
# See https://github.com/nxpkg/language-server-protocol/blob/master/extension-workspace-references.md.
...
type ReferenceConnection {
    ...
    # The search query (for Nxpkg search) that matches references in this connection.
    #
...
type ProductSubscriptionStatus {
    # The actual total number of users on this Nxpkg site.
    actualUserCount: Int!
    # The product license associated with this subscription, if any.
    license: ProductLicenseInfo
}

# Information about this site's product license (which activates certain Nxpkg features).
...
type ExtensionRegistry {
    ...
    # hostname (and port, if non-default HTTP/HTTPS) of the Nxpkg "appURL" site configuration property.
    #
    # It is null if extensions published on this Nxpkg site do not have an extension ID prefix.
    #
    # Examples: "nxpkg.example.com/", "nxpkg.example.com:1234/"
...
type ExtensionRegistryMutation {
    ...
    # extension.
    publishExtension(
        # The extension ID of the extension to publish. If a host prefix (e.g., "nxpkg.example.com/") is
...
type RegistryExtension implements Node {
    ...
    updatedAt: String
    # The URL to the extension on this Nxpkg site.
    url: String!
    ...
    registryName: String!
    # Whether the registry extension is published on this Nxpkg site.
    isLocal: Boolean!
...
# FOR INTERNAL USE ONLY.
type DotcomMutation {
    # Generates and returns a new Nxpkg license key (signed with Nxpkg.com's private key and verifiable
    # with the corresponding public key).
    #
    # Only Nxpkg.com site admins may perform this mutation.
    #
    # FOR INTERNAL USE ONLY.
    generateNxpkgLicenseKey(
