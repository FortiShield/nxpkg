...
import (
	...
	"github.com/nxpkg/nxpkg/cmd/frontend/internal/pkg/suspiciousnames"
...
// Org is DEPRECATED (but still in use by nxpkg/src). Use Node to look up an org by its
// graphql.ID instead.
...
func (*schemaResolver) CreateOrganization(ctx context.Context, args *struct {
	...
	_, err = db.OrgMembers.Create(ctx, newOrg.ID, currentUser.NxpkgID())
	if err != nil {
		return nil, err
	}
