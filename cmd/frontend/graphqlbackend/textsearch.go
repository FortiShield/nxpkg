...
import (
	...
	"github.com/nxpkg/nxpkg/cmd/frontend/internal/pkg/search"
	"github.com/nxpkg/nxpkg/cmd/frontend/internal/pkg/search/query"
	"github.com/nxpkg/nxpkg/cmd/frontend/types"
	"github.com/nxpkg/nxpkg/pkg/api"
	"github.com/nxpkg/nxpkg/pkg/endpoint"
	...
	zoektpkg "github.com/nxpkg/nxpkg/pkg/zoekt"
)
...
func zoektSearchHEAD(ctx context.Context, query *search.PatternInfo, repos []*search.RepositoryRevisions, useFullDeadline bool) (fm []*fileMatchResolver, limitHit bool, reposLimitHit map[string]struct{}, err error) {
	...
	// Repositories that weren't fully evaluated because they hit the Zoekt or Nxpkg file match limits.
	...
	if len(resp.Files) > int(query.FileMatchLimit) {
		// List of files we cut out from the Zoekt response because they exceed the file match limit on the Nxpkg end.
		...
		if !limitHit {
			...
			// than the limit we set on Nxpkg, so we cut out more results.

			// Generate a list of repositories that had results cut because they exceeded the file match limit set on Nxpkg.
