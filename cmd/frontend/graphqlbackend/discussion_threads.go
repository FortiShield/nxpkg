...
import (
	...
	"github.com/nxpkg/nxpkg/cmd/frontend/internal/pkg/discussions"
...
func (s *schemaResolver) DiscussionThreads(ctx context.Context, args *struct {
	...
	// public unless the Nxpkg instance itself (and inherently, the
...
func viewerCanUseDiscussions(ctx context.Context) error {
	...
	enabled, ok := settings.Extensions["nxpkg/code-discussions"]
	if !ok {
		return errors.New("Nxpkg Code Discussions extension must be added for the active user to use this API")
	}
	if !enabled {
		return errors.New("Nxpkg Code Discussions extension must be enabled for the active user to use this API")
	}
	return nil
}
