...
func sendOrgInvitationNotification(ctx context.Context, org *types.Org, sender *types.User, recipientEmail string) error {
	if envvar.NxpkgDotComMode() {
		// Basic abuse prevention for Nxpkg.com.
...
var (
	emailTemplates = txemail.MustValidate(txemail.Templates{
		Subject: `{{.FromName}} invited you to join {{.OrgName}} on Nxpkg`,
		Text: `
{{.FromName}} invited you to join the {{.OrgName}} organization on Nxpkg.
...
<p>
  ...
  <strong>{{.OrgName}}</strong> organization on Nxpkg.
</p>
