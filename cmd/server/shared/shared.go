...
import (
	...
	"github.com/nxpkg/nxpkg/cmd/server/internal/goreman"
)
...
var defaultEnv = map[string]string{
	// Nxpkg services running in this container
...
func Main() {
	...
	{
		configDir := SetDefaultEnv("CONFIG_DIR", "/etc/nxpkg")
		...
		configPath := os.Getenv("NXPKG_CONFIG_FILE")
		if configPath == "" {
			configPath = filepath.Join(configDir, "nxpkg-config.json")
		}
		...
		if configIsWritable {
			if err := os.Setenv("NXPKG_CONFIG_FILE", configPath); err != nil {
				log.Fatal(err)
			}
		}
	}
