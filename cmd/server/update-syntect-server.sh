...
docker pull nxpkg/syntect_server
docker run --rm nxpkg/syntect_server cat /syntect_server > syntect_server
...
gsutil cp -n syntect_server gs://nxpkg-artifacts/syntect_server/${HASH}
gsutil acl ch -u AllUsers:R gs://nxpkg-artifacts/syntect_server/${HASH}
rm syntect_server
...
echo "//docker:run curl -o /usr/local/bin/syntect_server https://storage.googleapis.com/nxpkg-artifacts/syntect_server/${HASH} && chmod +x /usr/local/bin/syntect_server"
