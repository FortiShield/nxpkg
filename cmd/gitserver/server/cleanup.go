...
import (
	...
	"github.com/nxpkg/nxpkg/pkg/gitserver/protocol"
...
// 3. Remove inactive repos on nxpkg.com
...
func (s *Server) cleanupRepos() {
	...
	maybeRemoveInactive := func(gitDir string) (done bool, err error) {
		...
		// disableAutoGitUpdates=true. This is true for nxpkg.com and
		// maybeRemoveInactive should only be run for nxpkg.com
	...
	if s.DeleteStaleRepositories {
		// Nxpkg.com can potentially clone all of github.com, so we
...
func getRecloneTime(gitDir string) (time.Time, error) {
	...
	update := func() (time.Time, error) {
		now := time.Now()
		cmd := exec.Command("git", "config", "--add", "nxpkg.recloneTimestamp", strconv.FormatInt(time.Now().Unix(), 10))
		cmd.Dir = gitDir
	...
	cmd := exec.Command("git", "config", "--get", "nxpkg.recloneTimestamp")
	cmd.Dir = gitDir
	out, err := cmd.Output()
	if err != nil {
