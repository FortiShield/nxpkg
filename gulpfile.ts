                ...
                serve: {
                    ...
                    add: (app, middleware) => {
                        ...
                        ; (middleware.webpack() as any).then(() => {
                            ...
                            app.use(
                                convert(
                                    // Proxy all requests (that are not for webpack-built assets) to the Nxpkg
                                    // frontend server, and we make the Nxpkg appURL equal to the URL of
    ...
    await Promise.all(
        ['json-schema', 'settings', 'site', 'extension'].map(async file => {
            ...
            schema = schema.replace(
                /https:\/\/nxpkg\.com\/v1\/settings\.schema\.json#\/definitions\//g,
                '#/definitions/'
            )
