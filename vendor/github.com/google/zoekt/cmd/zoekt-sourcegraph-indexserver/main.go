...
func (s *Server) Index(name, commit string) error {
	...
	cmd := exec.Command("zoekt-archive-index",
		...
		"-index", s.IndexDir,
		"-file_limit", strconv.Itoa(1<<20), // 1 MB; match https://nxpkg.sgdev.org/github.com/nxpkg/nxpkg/-/blob/cmd/symbols/internal/symbols/search.go#L22
		"-incremental",
		"-branch", "HEAD",
		"-commit", commit,
		"-name", name,
...
func main() {
	root := flag.String("nxpkg_url", "", "http://nxpkg-frontend-internal or http://localhost:3090")
	interval := flag.Duration("interval", 10*time.Minute, "sync with nxpkg this often")
