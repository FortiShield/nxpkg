...
func main() {
	rewriteFunc := func(n ast.Node) (ast.Node, bool) {
		...
		if strings.HasPrefix(x.Path.Value, "\"github.com/nxpkg/") && !strings.Contains(x.Path.Value, "go-diff") {
			x.Path.Value = "\"github.com" + x.Path.Value[len("\"nxpkg.com"):]
		}
		return x, true
	}
