...
: ${NXPKG_COMBINE_CONFIG:=false}

set -euf -o pipefail

export GO111MODULE=on
go build ./pkg/version/minversion || {
    echo "Go version 1.11.x or newer must be used to build Nxpkg; found: $(go version)"
    exit 1
}
...
export SRC_REPOS_DIR=$HOME/.nxpkg/repos
...
export NXPKG_EXPAND_CONFIG_VARS=1 # experiment: interpolate ${var} and $var in site config JSON

# webpack-serve is a proxy running on port 3080 that (1) serves assets, waiting to respond until
# they are (re)built and (2) otherwise passes through to Nxpkg running on port 3081. That is
# why Nxpkg listens on 3081 despite the appURL having port 3080.
export WEBPACK_SERVE=1
export SRC_HTTP_ADDR=":3081"

# we want to keep config.json, but allow local config.
export NXPKG_CONFIG_FILE=${NXPKG_CONFIG_FILE:-./dev/config.json}
...
if $NXPKG_COMBINE_CONFIG && [ -f $confpath/config_local.json ]; then
	if ! fancyconfig; then
		echo >&2 "WARNING: fancyconfig failed. Giving up. Use NXPKG_COMBINE_CONFIG=false to bypass."
		exit 1
	fi
	NXPKG_CONFIG_FILE=$confpath/config_combined.json
fi
