...
[ ! "$(docker volume ls | grep $db_volume)" ] && docker volume create --name $db_volume

# Create application data volume if not exists
[ ! "$(docker volume ls | grep $app_volume)" ] && docker volume create --name $app_volume
