...
CURRENT_DB_SCHEMA=$(psql -t -d nxpkg-test-db -c 'select version from schema_migrations' | xargs echo -n)
...
function runTest() {
	...
	EOF
        ...
        NOW_DB_SCHEMA=$(psql -t -d nxpkg-test-db -c 'select version from schema_migrations' | xargs echo -n)
