...
// Nxpkg application and writes it to stdout.
//
...
import (
	...
	bk "github.com/nxpkg/nxpkg/pkg/buildkite"
)
...
func pkgs() []string {
	...
	err := filepath.Walk(".", func(path string, info os.FileInfo, err error) error {
		...
		pkg, err := build.Import("github.com/nxpkg/nxpkg/"+path, "", 0)
		if err != nil {
...
func main() {
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr,
			`Generate a Buildkite YAML file that tests the entire Nxpkg application and write it to stdout.
`)
		flag.PrintDefaults()
	}
	flag.Parse()
	...
	addDockerImageStep := func(app string, insiders bool) {
		appBase := app
		cmdDir := "./cmd/" + appBase
		pkgPath := "github.com/nxpkg/nxpkg/cmd/" + appBase
		...
		if _, err := os.Stat(buildScript); err == nil {
			...
			cmds = append(cmds,
				bk.Cmd("go build github.com/nxpkg/nxpkg/vendor/github.com/nxpkg/godockerize"),
				bk.Cmd(fmt.Sprintf("./godockerize build -t %s:%s --go-build-flags='-ldflags' --go-build-flags='-X github.com/nxpkg/nxpkg/pkg/version.version=%s' --env VERSION=%s %s", image, version, version, version, pkgPath)),
			)
		}
		cmds = append(cmds,
	...
	for _, path := range pkgs() {
		...
		stepOpts := []bk.StepOpt{
			...
			bk.Cmd("go test ./" + path + " -v -race -coverprofile=" + coverageFile + " -covermode=atomic -coverpkg=github.com/nxpkg/nxpkg/..."),
	...
	addDeploySteps := func() {
		...
		if deploy {
			...
			pipeline.AddStep(":dog:",
				...
				bk.Concurrency(1),
				bk.Env("VERSION", version),
				bk.Env("CONTEXT", "gke_nxpkg-dev_us-central1-a_dogfood-cluster-7"),
