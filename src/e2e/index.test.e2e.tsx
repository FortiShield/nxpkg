...
describe('e2e test suite', () => {
    ...
    if (process.env.NXPKG_BASE_URL && process.env.NXPKG_BASE_URL !== 'http://localhost:3080') {
        baseURL = process.env.NXPKG_BASE_URL
        // Assume that the dogfood (nxpkg.sgdev.org) override token works.
    ...
    describe('Repository component', () => {
        ...
        describe('file tree', () => {
            it('does navigation on file click', async () => {
                await page.goto(
                    baseURL + '/github.com/nxpkg/godockerize@05bac79edd17c0f55127871fa9c6f4d91bebf07c'
                )
                await enableOrAddRepositoryIfNeeded()
                ...
                await assertWindowLocation(
                    '/github.com/nxpkg/godockerize@05bac79edd17c0f55127871fa9c6f4d91bebf07c/-/blob/godockerize.go'
                )
            })

            it('expands directory on row click (no navigation)', async () => {
                await page.goto(baseURL + '/github.com/nxpkg/jsonrpc2@c6c7b9aa99fb76ee5460ccd3912ba35d419d493d')
                await enableOrAddRepositoryIfNeeded()
                ...
                await assertWindowLocation('/github.com/nxpkg/jsonrpc2@c6c7b9aa99fb76ee5460ccd3912ba35d419d493d')
            })
            ...
            it('does navigation on directory row click', async () => {
                ...
                await page.waitForSelector('.tree__row--expanded [data-tree-path="websocket"]')
                await assertWindowLocation(
                    '/github.com/nxpkg/jsonrpc2@c6c7b9aa99fb76ee5460ccd3912ba35d419d493d/-/tree/websocket'
                )
            })
            ...
            it('responds to keyboard shortcuts', async () => {
                ...
                await page.goto(
                    baseURL +
                    '/github.com/nxpkg/go-diff@3f415a150aec0685cb81b73cc201e762e075006d/-/blob/.travis.yml'
                )
                await enableOrAddRepositoryIfNeeded()
        ...
        describe('rev resolution', () => {
            it('shows clone in progress interstitial page', async () => {
                await page.goto(baseURL + '/github.com/nxpkgtest/AlwaysCloningTest')
                await enableOrAddRepositoryIfNeeded()
            ...
            it('resolves default branch when unspecified', async () => {
                await page.goto(baseURL + '/github.com/nxpkg/go-diff/-/blob/diff/diff.go')
                await enableOrAddRepositoryIfNeeded()
            ...
            it('updates rev with switcher', async () => {
                await page.goto(baseURL + '/github.com/nxpkg/checkup/-/blob/s3.go')
                await enableOrAddRepositoryIfNeeded()
                // Open rev switcher
                await page.waitForSelector('.repo-header__rev')
                ...
                await assertWindowLocation('/github.com/nxpkg/checkup@v0.1.0/-/blob/s3.go')
            })
        })
        ...
        describe('hovers', () => {
            describe(`Blob`, () => {
                ...
                it.skip('gets displayed and updates URL when clicking on a token', async () => {
                    ...
                    await assertWindowLocation(
                        '/github.com/nxpkg/godockerize@05bac79edd17c0f55127871fa9c6f4d91bebf07c/-/blob/godockerize.go#L23:3'
                    )
                ...
                describe('jump to definition', () => {
                    it('noops when on the definition', async () => {
                        await page.goto(
                            baseURL +
                            '/github.com/nxpkg/go-diff@3f415a150aec0685cb81b73cc201e762e075006d/-/blob/diff/parse.go#L29:6'
                        )
                        await enableOrAddRepositoryIfNeeded()
                        await clickHoverJ2D()
                        await assertWindowLocation(
                    ...
                    it('does navigation (same repo, same file)', async () => {
                        await page.goto(
                            baseURL +
                            '/github.com/nxpkg/go-diff@3f415a150aec0685cb81b73cc201e762e075006d/-/blob/diff/parse.go#L25:10'
                        )
                        await enableOrAddRepositoryIfNeeded()
                        await clickHoverJ2D()
                    ...
                    it('does navigation (same repo, different file)', async () => {
                        await page.goto(
                            baseURL +
                            '/github.com/nxpkg/go-diff@3f415a150aec0685cb81b73cc201e762e075006d/-/blob/diff/print.go#L13:31'
                        )
                        await enableOrAddRepositoryIfNeeded()
                        await clickHoverJ2D()
                        await assertWindowLocation(
                            '/github.com/nxpkg/go-diff@3f415a150aec0685cb81b73cc201e762e075006d/-/blob/diff/diff.pb.go#L38:6'
                        )
                    ...
                    it('does navigation (external repo)', async () => {
                        await page.goto(
                            baseURL +
                            '/github.com/nxpkg/vcsstore@267289226b15e5b03adedc9746317455be96e44c/-/blob/server/diff.go#L27:30'
                        )
                        await enableOrAddRepositoryIfNeeded()
                        await clickHoverJ2D()
                        await assertWindowLocation(
                            '/github.com/nxpkg/go-vcs@aa7c38442c17a3387b8a21f566788d8555afedd0/-/blob/vcs/repository.go#L103:6'
                        )
                    })
                })
                ...
                describe('find references', () => {
                    it('opens widget and fetches local references', async function (): Promise<void> {
                        ...
                        await assertWindowLocation(
                            '/github.com/nxpkg/go-diff@3f415a150aec0685cb81b73cc201e762e075006d/-/blob/diff/parse.go#L29:6&tab=references'
                        )
                    ...
                    it('opens widget and fetches external references', async function (): Promise<void> {
                        ...
                        await page.goto(
                            baseURL +
                            '/github.com/nxpkg/go-diff@3f415a150aec0685cb81b73cc201e762e075006d/-/blob/diff/parse.go#L32:16&tab=references:external'
                        )
                        await enableOrAddRepositoryIfNeeded()
        ...
        describe('external code host links', () => {
            ...
            it.skip('on line blame', async () => {
                await page.goto(
                    baseURL +
                    '/github.com/nxpkg/go-diff@3f415a150aec0685cb81b73cc201e762e075006d/-/blob/diff/parse.go#L19'
                )
                await enableOrAddRepositoryIfNeeded()
                ...
                await assertWindowLocation(
                    '/github.com/nxpkg/go-diff/-/commit/f93a4e38b36b4003edbfdff66d3b92c6cd977c1c'
                )
            })
            ...
            it('on repo navbar ("View on GitHub")', async () => {
                ...
                await retry(async () =>
                    assert.equal(
                        ...
                        'https://github.com/nxpkg/go-diff/blob/3f415a150aec0685cb81b73cc201e762e075006d/diff/parse.go#L19'
                    )
                )
            })
        })
    ...
    describe('Search component', () => {
        it.skip('renders results for nxpkg/go-diff (no search group)', async () => {
            await page.goto(
                baseURL + '/search?q=diff+repo:nxpkg/go-diff%403f415a150aec0685cb81b73cc201e762e075006d+type:file'
            )
            ...
            await retry(async () => {
                assert.equal(
                    ...
                    baseURL +
                    '/github.com/nxpkg/go-diff@3f415a150aec0685cb81b73cc201e762e075006d/-/blob/diff/testdata/sample_file_extended_empty_rename.diff#L1:1a',
                    'Unexpected window.location.href after clicking result'
                )
            })
        })

        it('accepts query for nxpkg/jsonrpc2', async () => {
            await page.goto(baseURL + '/search')

            // Update the input value
            await page.waitForSelector('input.query-input2__input')
            await page.keyboard.type('test repo:nxpkg/jsonrpc2@c6c7b9aa99fb76ee5460ccd3912ba35d419d493d')
