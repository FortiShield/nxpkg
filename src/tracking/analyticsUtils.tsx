...
/**
 * Indicates if the webapp ever receives a message from the user's Nxpkg browser extension,
 ...
 */
export const browserExtensionMessageReceived = (document.getElementById('nxpkg-app-background')
    ? // If the marker exists, the extension is installed
    of(true)
    : // If not, listen for a registration event
    fromEvent<CustomEvent>(document, 'nxpkg:browser-extension-registration').pipe(
        take(1),
        mapTo(true)
    )
).pipe(
