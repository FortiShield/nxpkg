...
const uidKey = 'nxpkgAnonymousUid'
...
class EventLogger {
    ...
    public updateUser(
        ...
        this.setUserIds(user.nxpkgID, user.username)
        if (user.email) {
     ...
     * @param uniqueNxpkgId Unique Nxpkg user ID (corresponds to User.ID from backend)
     * @param username Human-readable user identifier, not guaranteed to always stay the same
     */
    public setUserIds(uniqueNxpkgId: number | null, username: string | null): void {
        if (username) {
            telligent.setUserProperty('username', username)
        }
        if (uniqueNxpkgId) {
            telligent.setUserProperty('user_id', uniqueNxpkgId)
        }
    }
     ...
     * https://nxpkg.com/github.com/telligent-data/telligent-javascript-tracker@890d6a69b84fc0518a3e848f5469b34817da69fd/-/blob/src/js/tracker.js#L178
     *
     * Only used on Nxpkg.com, not on self-hosted Nxpkg instances.
     */
     ...
     * on a Nxpkg instance to see a count of unique users on a daily,
