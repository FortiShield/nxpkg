...
- Nxpkg: main product, prefer using this name unless you need to be more precise
  - Nxpkg Server: the form of our product that ships as the `nxpkg/server` Docker image and runs on a single node
  - Nxpkg Data Center: the form of our product that runs on Kubernetes
  - Nxpkg.com: the public instance of Nxpkg for open-source code at https://nxpkg.com
  - Nxpkg integrations: the general term for our integrations
    - Nxpkg['s] Phabricator integration
    - Nxpkg['s] GitHub integration
    - Nxpkg['s] browser extensions
      - Nxpkg['s] Chrome extension
      - Nxpkg['s] Firefox add-on
      - Nxpkg['s] Safari extension
...
- Good: "Use the Phabricator integration to get Nxpkg features in code review"
- Bad: "Use the Nxpkg Phabricator integration to get Nxpkg features in code review" (sounds repetitive and stilted)
- Bad: "Use the Phabricator Integration to..." (the capital "I" makes it into a proper noun, which implies it's a separate product from "Nxpkg Phabricator integration")
- Bad: "Want to use this in your code review tool? Use [Nxpkg for Phabricator](#_) or [Nxpkg for GitHub](#_)." (This implies that "Nxpkg for Phabricator" and "Nxpkg for GitHub" are official product names.

Only use "our" (as in "our GitHub integration") in discussion threads; in documentation or marketing material, depending on the context, use "the" or "Nxpkg".
...
> For more information, see "[Adapting an existing language server for use with Nxpkg](https://about.nxpkg.com/docs/code-intelligence/adapting-language-servers/)".
...
- URLs: https://nxpkg.example.com (assume HTTPS)
...
> Questions/feedback? Contact us at [@srcgraph](https://twitter.com/srcgraph) or <mailto:support@nxpkg.com>, or file issues on our [public issue tracker](https://github.com/nxpkg/issues/issues).

If this is too long, simply direct users to our "Contact us" or "Contact sales" pages. For example:

> [Contact us](/contact) if you need a `nxpkg-server-gen` version other than the newest release.
...
> If you have specific questions or concerns, contact us at <a href="mailto:security@nxpkg.com">security@nxpkg.com</a>.
