# Getting started with developing Nxpkg

The best way to become familiar with the Nxpkg repository is by
reading the code at https://nxpkg.com/github.com/nxpkg/nxpkg.

## Environment

The Nxpkg server is actually a collection of smaller binaries, each of
which performs one task. The core entrypoint for the Nxpkg development
...
Run this command to get the Nxpkg source code on your local machine:

```
go get github.com/nxpkg/nxpkg
```

This is your "Nxpkg repository directory".
...
You need to have the redis image running when you run the Nxpkg
...
### II. Create the Nxpkg user and password
...
The Nxpkg server reads PostgreSQL connection
...
that contains the Nxpkg source code, and run:
...
instance of Nxpkg. Navigate your browser to http://localhost:3080 to
...
While developing Nxpkg, you may run into:

`frontend | failed to migrate the DB. Please contact hi@nxpkg.com for further assistance:Dirty database version 1514702776. Fix and force version.`
...
```bash
# increase max number of file descriptors for running a nxpkg instance.
ulimit -n 10000
```
...
See [testing.md](/nxpkg/nxpkg/blob/master/docs/testing.md) for details.
...
The Nxpkg repository relies on code generation triggered by `go generate`. Code generation is used for a variety of tasks:
...
Running Nxpkg on Windows is not actively tested, but should be possible within the Windows Subsystem for Linux (WSL).
Nxpkg currently relies on Unix specifics in several places, which makes it currently not possible to run Nxpkg directly inside Windows without WSL.
